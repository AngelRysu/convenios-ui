...
import { createRecord, getData, getBinaryData, deleteRecord } from '~/utils/apiUtils'; 
...
export default function AnexosPaso4({ setPaso, tipoOrganizacion, folio, idConvenio }: AnexosProps) {
  ...
  const handleDelete = async (key: string) => {
    ...
    if (!window.confirm(`¿Estás seguro de que deseas eliminar permanentemente "${docToDelete.nombre}"? Esta acción no se puede deshacer.`)) return;

    setIsLoading(true);
    try {
        const result = await deleteRecord({ endpoint: `/media/anexo/${docToDelete.idAnexo}` });

        if (result.statusCode >= 200 && result.statusCode < 300) {
            alert("Documento eliminado exitosamente.");
            updateDocumentState(key, resetDoc);
        } else {
            console.error("Error al eliminar documento:", result.errorMessage);
            alert(`Error al eliminar: ${result.errorMessage}`);
        }
    } catch (error) {
        console.error("Error inesperado al eliminar:", error);
        alert("Ocurrió un error inesperado al intentar eliminar el documento.");
    } finally {
        setIsLoading(false);
    }
  };
  ...
  const handleView = (key: string) => {
    ...
    try {
        ...
        if (!newWindow) alert('El navegador bloqueó la apertura de la vista previa.');
  ...
  const handleSave = async () => {
    ...
    if (archivosAsubir.length === 0) return alert("No hay archivos nuevos o reemplazados pendientes de guardar.");
    ...
    try {
        ...
        if (result.statusCode >= 200 && result.statusCode < 300) {
            alert(`Documentos de ${tipoOrganizacion} guardados correctamente.`);
            await fetchSavedDocuments();
        } else {
            console.error("Error al guardar documentos:", result.errorMessage);
            alert(`Error al guardar los archivos: ${result.errorMessage}`);
        }
    } catch (error) {
  ...
  const handleContinue = () => {
    if (todosSubidos) setPaso(5);
    else alert("Por favor, sube y **guarda** todos los documentos requeridos antes de continuar.");
  }
  ...
  return (
    <Box sx={{ width: '100%', p: 3 }}>
        ...
        <Box sx={{ width: "100%", display: "flex", justifyContent: "space-between", mt: 4, pt: 2, borderTop: '1px solid #cacacaff' }}>
            <Button variant="outlined" color="primary" startIcon={<ArrowBack />} onClick={() => setPaso(3)} disabled={isLoading} sx={{ ...commonBtnSx, ml: 2 }}>
                Regresar
            </Button>
            ...
            <Box>
                ...
                <Button
                  variant="contained" color="primary" endIcon={<ArrowForward />} onClick={handleContinue} 
